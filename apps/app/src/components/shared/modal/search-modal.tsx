...
import { getWebUrlBySpaceId } from '@/lib/space-utils';
...
export function SearchModal() {
    ...
    // Helper function to get the correct web URL
    const getWebUrl = (web: Web) => {
        return `/trails/${web.id}`;
    };

    const filteredWebs = useMemo(() => {
        if (!query.trim()) return webs.slice(0, 20);

        return webs.filter(web => {
            const searchText = `${web.title || ''} ${web.url} ${web.prompt || ''}`.toLowerCase();
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            } else if (e.key === 'Enter' && filteredWebs[selectedIndex]) {
                e.preventDefault();
                window.location.href = getWebUrl(filteredWebs[selectedIndex]);
                setIsOpen(false);
            }
        };
    ...
    }, [shouldOpen, filteredWebs, selectedIndex, setIsOpen, getWebUrl]);
                                                        ...
                                                        <div className="space-y-2">
                                                            <div className="flex items-center justify-between gap-3">
                                                                <h3 className="text-sm font-medium truncate">
                                                                    {web.title || extractDomain(web.url)}
                                                                </h3>
                                                                <span className={cn(
                                                                    "text-xs px-2 py-1 rounded border",
                                                                    web.status === 'COMPLETE' && "text-green-600 bg-green-600/10 border-green-600/20",
                                                                    web.status === 'PENDING' && "text-yellow-600 bg-yellow-600/10 border-yellow-600/20",
                                                                    web.status === 'PROCESSING' && "text-blue-600 bg-blue-600/10 border-blue-600/20",
                                                                    web.status === 'FAILED' && "text-red-600 bg-red-600/10 border-red-600/20"
                                                                )}>
                                                                    {web.status}
                                                                </span>
                                                            </div>
                                                            <div className="text-xs text-muted-foreground flex items-center justify-between">
                                                                <span className="truncate">{extractDomain(web.url)}</span>
                                        ...
                                        <div className="flex-1 flex items-center justify-center p-8">
                                            <div className="text-center">
                                                ...
                                                <p className="text-sm text-muted-foreground font-mono">
                                                    {query ? 'No webs found' : 'Start typing to search...'}
                                                </p>
                                            </div>
                                        </div>
                                    )}
